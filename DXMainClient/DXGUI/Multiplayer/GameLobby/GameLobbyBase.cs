    ...
    public abstract class GameLobbyBase : INItializableWindow
    {
        ...
        protected virtual void CopyPlayerDataToUI()
        {
            ...
            for (int pId = 0; pId < Players.Count; pId++)
            {
                ...
                if (GameModeMap != null)
                {
                    ddPlayerTeams[pId].AllowDropDown = !playerExtraOptions.IsForceRandomTeams && allowPlayerOptionsChange && !GameModeMap.IsCoop && !GameModeMap.ForceNoTeams;
                    ddPlayerStarts[pId].AllowDropDown = !playerExtraOptions.IsForceRandomStarts && allowPlayerOptionsChange && (GameModeMap.IsCoop || !GameModeMap.ForceRandomStartLocations);
                }
            }
            ...
            for (int aiId = 0; aiId < AIPlayers.Count; aiId++)
            {
                ...
                if (GameModeMap != null)
                {
                    ddPlayerTeams[index].AllowDropDown = !playerExtraOptions.IsForceRandomTeams && allowOptionsChange && !GameModeMap.IsCoop && !GameModeMap.ForceNoTeams;
                    ddPlayerStarts[index].AllowDropDown = !playerExtraOptions.IsForceRandomStarts && allowOptionsChange && (GameModeMap.IsCoop || !GameModeMap.ForceRandomStartLocations);
                }
            }
        ...
        protected virtual void ChangeMap(GameModeMap gameModeMap)
        {
            ...
            foreach (PlayerInfo pInfo in concatPlayerList)
            {
                if (!GameModeMap.AllowedStartingLocations.Contains(pInfo.StartingLocation) ||
                    (!GameModeMap.IsCoop && GameModeMap.ForceRandomStartLocations))
                    pInfo.StartingLocation = 0;
