...
namespace DTAClient.DXGUI.Multiplayer
{
    ...
    internal class LANLobbyBroadcastManager : IDisposable
    {
        ...
        public LANLobbyBroadcastManager(int lobbyPort, Encoding encoding, Action<Action<string, IPEndPoint>, string, IPEndPoint>? addCallback = null)
        {
        ...
        private void Listen()
        {
            try
            {
                while (true)
                {
                    ...
                    if (addCallback != null)
                    {
                        addCallback(HandleNetworkMessage, data, ipEndPoint);
                    }
                    else
                    {
                        // No callback provided, invoke event directly (less safe for UI operations)
